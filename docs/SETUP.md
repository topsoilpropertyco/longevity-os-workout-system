...
There are eleven, numbered in order: `0001_init` (tables, enums, indexes, triggers), `0002_rls` (RLS on every table), `0003_seed_equipment`, `0004_seed_presets` (Home, Planet Fitness — standard, CrossFit box — typical, Bodyweight only), `0005_cron` (the scheduled jobs), `0006_worker`, `0007_equipment_vocabulary`, `0008_oura_oauth`, `0009_grants`, `0010_program_phases` (phases, weekday templates, `start_program`) and `0011_seed_kot` (the Knees Over Toes program itself — 69 steps, 3 phases, 12 weekday templates, generated from `programs/kot/program.json`). They are idempotent, so re-running one is harmless. `supabase/README.md` has the full per-file detail and the local CLI loop.

**The simple way, no CLI, no Docker, one paste:**

`supabase/SCHEMA_ALL_IN_ONE.sql` is every migration concatenated in order, regenerated by `npm run sql:bundle` and checked by CI against the files it came from.

1. Open the raw file on GitHub, **Cmd+A**, **Cmd+C**. Use a URL pinned to a commit rather than to `main` — GitHub's raw CDN caches `main` for a few minutes, and pasting a stale copy is a confusing way to lose ten minutes.
2. In the Supabase dashboard, open **SQL Editor → New query**, paste, **Run**.
3. You want **"Success. No rows returned."**

That paste runs as a **single transaction**, which is not how `psql` runs the same files — it gives each statement its own. The difference is not academic: `alter type … add value` followed by a row using that value is legal across two transactions and illegal inside one (SQLSTATE 55P04). CI tests the bundle both ways for exactly this reason.

Then confirm, in the dashboard:

- **Table Editor** — `users`, `locations`, `equipment_catalog`, `exercises`, `sessions`, `sets`, `oura_daily`, `llm_jobs` and the rest of PRD §10.
- **Authentication → Policies** — RLS enabled on every table. `0002_rls.sql` fails loudly if one slipped through, so a clean run is itself the check.

**With the CLI** (needs Docker): `npx supabase start` then `npx supabase db reset`. See `supabase/README.md`.

---

**Then fill it.** The migrations seed the two things small enough to be SQL — the equipment catalog and the KOT program. The exercise library is 1,938 records with their media, which as an INSERT script would be a megabyte nobody could paste. It goes over the wire instead:

```bash
npm run ingest:exercises          # build the library, once
npm run seed -- --dry-run         # say what would be written
npm run seed                      # write it
```

`ingest:exercises` joins `yuhonas/free-exercise-db` (the metadata spine) with the Gym Visual media set, normalizes equipment into the engine's vocabulary, and prints a **media coverage report** plus a list of KOT movements with no GIF. Expect gaps on tibialis raise, Patrick step, Poliquin step, ATG split squat, reverse Nordic, QL extension and seated good morning — those fall back to a static image plus written cues (RESEARCH §1).

`seed` needs `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` (Step 5). It writes **reference data only** — rows with `user_id is null`, shared by every athlete — so it never touches a session, a log, a measurement or a token, and it is safe to re-run every time the library is regenerated.

Once you know your own id in `public.users`, one more run does the per-athlete setup — the three locations, and enrolment in Knees Over Toes at phase one, week one:

```bash
npm run seed -- --user <your uuid from public.users>
```

Both halves are idempotent. `apply_location_preset` and `start_program` are written so a second call changes nothing; in particular `start_program` returns the existing enrolment rather than resetting anyone to week 1.

> The service role key bypasses every RLS policy in the database. It belongs in `.env.local` and in Vercel's environment, and nowhere else — never in a browser, never in a commit, never in a chat window.
...
Runs the whole pipeline in the terminal — real Oura, the real engine, the real program — and prints today's session the way the app shows it. `--minutes=15`, `--location=pf`, `--date=2026-09-14`, `--no-oura` and `--json` all do what they look like.

It is the fastest way to answer "is the system actually working", and it needs neither a deployment nor a database: every integration that is missing prints one line saying so and the day is planned anyway.
