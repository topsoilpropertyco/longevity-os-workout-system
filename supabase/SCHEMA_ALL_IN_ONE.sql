...
--
-- 3 phases · 70 steps · 12 weekday templates.
...
insert into kot_incoming values
  ('zero-bodyweight-walk-warm-up', 1, 'Bodyweight Walk (Warm-Up)', '5–10 min — Easy pace, not a training stimulus.', '{"duration_min":5}'::jsonb, 'zone-2-steady', '[{"equipment_missing":"outdoor_route","note":"Planet Fitness or bad weather: treadmill at an easy pace.","use_slug":"walking-treadmill"}]'::jsonb, '{}'::text[], 'warm_up', 'zero', '[]'::jsonb, null, false, null, null),
  ('zero-backward-walking', 2, 'Backward Walking', '10 min — The method’s knee entry point, and the one movement of Seth’s that runs in every session.', '{"duration_min":10}'::jsonb, 'backward-walk-outdoors', '[{"equipment_missing":"outdoor_route","note":"Planet Fitness or bad weather: the same walk on a treadmill with the belt POWERED OFF, driving the belt by foot.","use_slug":"backward-treadmill-walk"}]'::jsonb, '{}'::text[], 'warm_up', 'zero', '["Flat ground, hands free, glancing over alternating shoulders.","A shallow incline — a ramp or a gentle hill.","Weight vest, or a backpack.","Backward sled drag, wherever there is a sled."]'::jsonb, null, false, null, null),
  ('zero-tibialis-raise', 3, 'Tibialis Raise', '25 reps', '{"reps":25}'::jsonb, 'tibialis-raise', '[{"equipment_missing":"wall_space","note":"No wall to lean on: anchor a band low in front and loop it over the forefoot.","use_slug":"tibialis-raise-band"},{"equipment_missing":"tibialis_bar","note":"No tib bar at either gym: stand a dumbbell on end and pinch it between the feet.","use_slug":"tibialis-raise-dumbbell"}]'::jsonb, '{}'::text[], 'lower_legs', 'zero', '["Stand closer to the wall — less load, easier.","Step farther from the wall — more load, harder.","Add load: a dumbbell held between the feet, or a band over the forefoot.","Tib bar, once one is available."]'::jsonb, null, false, null, null),
  ('zero-calf-raise-slant-board', 4, 'Calf Raise (Slant Board)', '25 reps', '{"reps":25}'::jsonb, 'fhl-calf-raise', '[{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"fhl-calf-raise"}]'::jsonb, '{}'::text[], 'lower_legs', 'zero', '["Both feet, full range: heels below the platform at the bottom.","One leg at a time (wrap the free leg behind).","Add a weight vest or hold a dumbbell."]'::jsonb, null, false, null, null),
  ('zero-kot-calf-raise', 5, 'KOT Calf Raise', '25 reps — The checklist says "as prescribed"; the reps come from the Knee Ability Zero recap.', '{"reps":25}'::jsonb, 'kot-seated-calf-raise', '[{"equipment_missing":"bench_flat","note":"Nowhere to sit: the standing big-toe calf raise trains the same tissue, with the knee straight.","use_slug":"fhl-calf-raise"}]'::jsonb, '{}'::text[], 'lower_legs', 'zero', '["Small knee bend, both feet.","Deeper knee bend as the ankle allows — heels lift slightly at the bottom.","One leg at a time.","Add a weight vest."]'::jsonb, null, false, null, null),
  ('zero-patrick-step-slant-board', 6, 'Patrick Step (Slant Board)', '25 reps, per side', '{"reps":25}'::jsonb, 'patrick-step', '[{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"patrick-step"}]'::jsonb, '{}'::text[], 'knee_ability', 'zero', '["Hold a wall or rail for balance.","Free-standing.","Reach the free leg farther forward.","Raise the standing surface.","Add load."]'::jsonb, null, true, null, null),
  ('zero-atg-split-squat', 7, 'ATG Split Squat', '25 reps, per side', '{"reps":25}'::jsonb, 'atg-split-squat', '[{"equipment_missing":"adjustable_dumbbell","note":"Travelling: bodyweight, or a loaded backpack.","use_slug":"atg-split-squat"}]'::jsonb, '{}'::text[], 'knee_ability', 'zero', '["Front foot elevated, holding a rail for assistance.","Front foot elevated, no hands.","Flat ground, no hands, back knee to the floor.","Dumbbells in both hands, toward 25% bodyweight per hand."]'::jsonb, null, true, 30, '{"weekly_increment_pct":2.5}'::jsonb),
  ('zero-nordic-curl', 8, 'Nordic Curl', '5 reps', '{"reps":5}'::jsonb, 'nordic-hamstring-curl', '[{"equipment_missing":"nordic_support","note":"Planet Fitness has no GHD and nothing safe to anchor the ankles under: run the seated leg curl machine instead.","use_slug":"seated-leg-curl"},{"equipment_missing":"nordic_support","note":"Travelling: reverse Nordic needs no anchor and keeps the eccentric quality.","use_slug":"reverse-nordic"}]'::jsonb, '{}'::text[], 'posterior_chain', 'zero', '["Lower a short way, hands catch early.","Lower farther each week, hands catch late.","Full lower, push back up with the hands.","Full rep down and up, no hands — the standard is 10."]'::jsonb, null, false, null, null),
  ('zero-l-sit', 9, 'L-Sit', '60 s hold', '{"hold_s":60}'::jsonb, 'l-sit', '[]'::jsonb, '{}'::text[], 'hip_flexors_core', 'zero', '["Level 1 — alternate lifting one leg at a time, seated, for the full time.","Level 2 — same, with the hips off the floor.","Level 3 — full L-sit, both legs and hips off the floor."]'::jsonb, null, false, null, null),
  ('zero-elephant-walk', 10, 'Elephant Walk', '25 reps', '{"reps":25}'::jsonb, 'elephant-walk', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'zero', '["Hands well forward, on fingertips or a box, knees bent.","Alternate straightening one leg at a time.","Walk the hands back until the palms reach the floor in front of the toes."]'::jsonb, null, false, null, null),
  ('zero-couch-stretch', 11, 'Couch Stretch', '60 s hold, per side', '{"hold_s":60}'::jsonb, 'couch-stretch', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'zero', '["Knee down, torso upright, hands on the floor.","Hands to the front thigh.","Hands to the hips.","Shoulders to the wall."]'::jsonb, null, true, null, null),
  ('zero-standing-pigeon', 12, 'Standing Pigeon', '2 sets, 90 s hold, per side — The checklist says only "hold each side". The 2 x 90 s comes from the Knee Ability Zero recap, where the same slot is a seated piriformis stretch.', '{"hold_s":90,"sets":2}'::jsonb, 'seated-piriformis-stretch', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'zero', '[]'::jsonb, null, true, null, null),
  ('zero-body-squat-slant-board', 13, 'Body Squat (Slant Board)', '5 × 5 reps — The checklist says "as prescribed"; the sets and reps come from the Knee Ability Zero recap, where this step is explicitly optional.', '{"reps":5,"sets":5}'::jsonb, 'bodyweight-squat', '[{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"bodyweight-squat"}]'::jsonb, '{}'::text[], 'knee_ability', 'zero', '["Partial depth, heels on the board.","Full depth.","More sets before more depth."]'::jsonb, null, false, 30, null),
  ('dense-bodyweight-walk-warm-up', 14, 'Bodyweight Walk (Warm-Up)', '5–10 min — Easy pace, not a training stimulus.', '{"duration_min":5}'::jsonb, 'zone-2-steady', '[{"equipment_missing":"outdoor_route","note":"Planet Fitness or bad weather: treadmill at an easy pace.","use_slug":"walking-treadmill"}]'::jsonb, '{}'::text[], 'warm_up', 'dense', '[]'::jsonb, null, false, null, null),
  ('dense-backward-sled-drag', 15, 'Backward Sled Drag', '10 min, 50% bodyweight — Neither of Seth’s gyms owns a sled, so this resolves to backward walking most weeks; the load is what it becomes when one is available.', '{"duration_min":10,"pct_bodyweight":0.5}'::jsonb, 'backward-sled-drag', '[{"equipment_missing":"sled","note":"No sled at Planet Fitness: backward walking on a treadmill with the belt POWERED OFF.","use_slug":"backward-treadmill-walk"},{"equipment_missing":"sled","note":"No sled and no treadmill at home: backward walking outdoors, unloaded.","use_slug":"backward-walk-outdoors"}]'::jsonb, '{}'::text[], 'warm_up', 'dense', '["Backward walking, unloaded, on flat ground.","Sled at roughly 50% of bodyweight for 10 minutes.","More load, same 10 minutes."]'::jsonb, null, false, null, null),
  ('dense-patrick-step', 16, 'Patrick Step', '10 × 10 reps, 20 min, per side — Ten sets of ten inside twenty minutes at the current load before the load goes up.', '{"duration_min":20,"reps":10,"sets":10}'::jsonb, 'patrick-step', '[{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"patrick-step"}]'::jsonb, '{}'::text[], 'knee_ability', 'dense', '["Bodyweight.","Held dumbbells.","Raise the standing surface for more range."]'::jsonb, null, true, null, null),
  ('dense-seated-good-morning', 17, 'Seated Good Morning', '3 sets, 5 min (reps 20, 10, 5)', '{"duration_min":5,"sets":3}'::jsonb, 'seated-good-morning', '[{"equipment_missing":"barbell","note":"Neither gym has a barbell: Planet Fitness has the Smith machine and a seated good-morning lever.","use_slug":"lever-seated-good-morning"},{"equipment_missing":"barbell","note":"Home: hold a single heavy dumbbell at the chest and hinge from the hips.","use_slug":"seated-good-mornings"}]'::jsonb, '{}'::text[], 'posterior_chain', 'dense', '["Bodyweight hinge, hands behind the head.","Light bar.","Toward 50% bodyweight, abs to the bench."]'::jsonb, null, false, null, null),
  ('dense-tibialis-raise', 18, 'Tibialis Raise', '4 sets, 5 min (reps 20, 15, 10, 5)', '{"duration_min":5,"sets":4}'::jsonb, 'tibialis-raise', '[{"equipment_missing":"wall_space","note":"No wall to lean on: anchor a band low in front and loop it over the forefoot.","use_slug":"tibialis-raise-band"},{"equipment_missing":"tibialis_bar","note":"No tib bar at either gym: stand a dumbbell on end and pinch it between the feet.","use_slug":"tibialis-raise-dumbbell"}]'::jsonb, '{}'::text[], 'lower_legs', 'dense', '["Stand closer to the wall — less load, easier.","Step farther from the wall — more load, harder.","Add load: a dumbbell held between the feet, or a band over the forefoot.","Tib bar, once one is available."]'::jsonb, 'https://www.youtube.com/watch?v=__AsD5K0i3Y&ab_channel=PerformanceHerts', false, null, null),
  ('dense-chin-up', 19, 'Chin-Up', '5 min', '{"duration_min":5}'::jsonb, 'chin-up', '[{"equipment_missing":"pull_up_bar","note":"Planet Fitness has no free bar: the assisted pull-up machine.","use_slug":"assisted-standing-chin-up"},{"equipment_missing":"pull_up_bar","note":"Planet Fitness alternative: lat pulldown.","use_slug":"cable-bar-lateral-pulldown"}]'::jsonb, '{}'::text[], 'upper_body', 'dense', '["Band- or machine-assisted.","Bodyweight.","Weighted."]'::jsonb, null, false, null, null),
  ('dense-dips', 20, 'Dips', '5 min', '{"duration_min":5}'::jsonb, 'chest-dip', '[{"equipment_missing":"dip_station","note":"Neither location has a dip station: bench dips between two benches.","use_slug":"bench-dips"}]'::jsonb, '{}'::text[], 'upper_body', 'dense', '["Bench dips.","Assisted parallel-bar dips.","Full-depth bodyweight dips."]'::jsonb, null, false, null, null),
  ('dense-smith-machine-curl', 21, 'Smith Machine Curl', '5 min', '{"duration_min":5}'::jsonb, 'smith-machine-bicep-curl', '[{"equipment_missing":"smith_machine","note":"Home has no Smith machine: dumbbell curls.","use_slug":"dumbbell-bicep-curl"}]'::jsonb, '{}'::text[], 'upper_body', 'dense', '[]'::jsonb, 'https://vimeo.com/749630580', false, null, null),
  ('dense-french-press', 22, 'French Press', '5 min', '{"duration_min":5}'::jsonb, 'barbell-lying-triceps-extension-skull-crusher', '[{"equipment_missing":"ez_curl_bar","note":"Home has no EZ bar: dumbbells.","use_slug":"dumbbell-lying-triceps-extension"}]'::jsonb, '{}'::text[], 'upper_body', 'dense', '[]'::jsonb, 'https://vimeo.com/749630580', false, null, null),
  ('dense-jefferson-curl-slant-board', 23, 'Jefferson Curl (Slant Board)', '5 reps, 25% bodyweight — Loaded spinal flexion — check it against the standing low-back injury before prescribing.', '{"pct_bodyweight":0.25,"reps":5}'::jsonb, 'jefferson-curl', '[{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"jefferson-curl"}]'::jsonb, '{}'::text[], 'posterior_chain', 'dense', '["Unloaded roll-down.","Light dumbbell.","Toward 25% bodyweight for 10 reps."]'::jsonb, null, false, null, null),
  ('dense-slant-board-calf-raise-loaded', 24, 'Slant Board Calf Raise (Loaded)', '10 reps', '{"reps":10}'::jsonb, 'fhl-calf-raise', '[{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"fhl-calf-raise"}]'::jsonb, '{}'::text[], 'lower_legs', 'dense', '["Bodyweight both feet.","Bodyweight one leg.","Loaded one leg."]'::jsonb, null, false, null, null),
  ('dense-tibialis-stretch', 25, 'Tibialis Stretch', '60 s hold', '{"hold_s":60}'::jsonb, 'posterior-tibialis-stretch', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'dense', '[]'::jsonb, null, false, null, null),
  ('dense-atg-split-squat', 26, 'ATG Split Squat', '10 × 5 reps, 20 min, per side', '{"duration_min":20,"reps":5,"sets":10}'::jsonb, 'atg-split-squat', '[{"equipment_missing":"adjustable_dumbbell","note":"Travelling: bodyweight, or a loaded backpack.","use_slug":"atg-split-squat"}]'::jsonb, '{}'::text[], 'knee_ability', 'dense', '["Front foot elevated, holding a rail for assistance.","Front foot elevated, no hands.","Flat ground, no hands, back knee to the floor.","Dumbbells in both hands, toward 25% bodyweight per hand."]'::jsonb, null, true, 30, '{"weekly_increment_pct":2.5}'::jsonb),
  ('dense-hip-flexor-tri-set', 27, 'Hip Flexor Tri-Set', '5 min — Three drills in one slot. The checklist says alternate them to failure; the spreadsheet says pick one per five minutes.', '{"duration_min":5}'::jsonb, 'l-sit', '[]'::jsonb, '{}'::text[], 'hip_flexors_core', 'dense', '["Dumbbell foot raise, no breaks.","Reverse squat at 50% bodyweight.","L-sit, maximum time off the ground."]'::jsonb, null, false, null, null),
  ('dense-single-leg-calf-raise', 28, 'Single-Leg Calf Raise', '5 min, per side', '{"duration_min":5}'::jsonb, 'single-leg-calf-raise', '[{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"single-leg-calf-raise"}]'::jsonb, '{}'::text[], 'lower_legs', 'dense', '["Bodyweight, floor.","Bodyweight, heel below a step or board.","Loaded, one dumbbell in the same-side hand."]'::jsonb, null, true, null, null),
  ('dense-bench-pullover', 29, 'Bench Pullover', '5 min, 25% bodyweight', '{"duration_min":5,"pct_bodyweight":0.25}'::jsonb, 'bent-arm-dumbbell-pullover', '[]'::jsonb, '{}'::text[], 'upper_body', 'dense', '["Along the bench.","Across the bench, hips low, for the full overhead stretch.","Toward 25% bodyweight."]'::jsonb, null, false, null, null),
  ('dense-atg-shoulder-press', 30, 'ATG Shoulder Press', '5 min, 50% bodyweight', '{"duration_min":5,"pct_bodyweight":0.5}'::jsonb, 'dumbbell-shoulder-press', '[]'::jsonb, '{}'::text[], 'upper_body', 'dense', '[]'::jsonb, null, false, null, null),
  ('dense-shoulder-external-rotation', 31, 'Shoulder External Rotation', '5 min, per side — Roughly 10% of bodyweight is the target; lighter is fine.', '{"duration_min":5}'::jsonb, 'external-rotation', '[{"equipment_missing":"cable_machine","note":"Home: band external rotation, elbow pinned to the side.","use_slug":"external-rotation-with-band"}]'::jsonb, '{}'::text[], 'upper_body', 'dense', '[]'::jsonb, null, true, null, null),
  ('dense-couch-stretch', 32, 'Couch Stretch', '60 s hold, per side', '{"hold_s":60}'::jsonb, 'couch-stretch', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'dense', '["Knee down, torso upright, hands on the floor.","Hands to the front thigh.","Hands to the hips.","Shoulders to the wall."]'::jsonb, null, true, null, null),
  ('dense-piriformis-stretch', 33, 'Piriformis Stretch', '60 s hold, per side', '{"hold_s":60}'::jsonb, 'seated-piriformis-stretch', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'dense', '[]'::jsonb, null, true, null, null),
  ('dense-butterfly-stretch', 34, 'Butterfly Stretch', '60 s hold', '{"hold_s":60}'::jsonb, 'butterfly-yoga-pose', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'dense', '[]'::jsonb, null, false, null, null),
  ('dense-vmo-squat', 35, 'VMO Squat', '10 × 10 reps, 10 min', '{"duration_min":10,"reps":10,"sets":10}'::jsonb, 'sissy-squat', '[{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"sissy-squat"}]'::jsonb, '{}'::text[], 'knee_ability', 'dense', '["Bodyweight, heels elevated (weeks 1–2).","Goblet-held dumbbell at 5% bodyweight (week 3).","+5% bodyweight per week.","Bar on the back at 5% bodyweight from week 8."]'::jsonb, null, false, null, null),
  ('dense-kot-squat-eccentric', 36, 'KOT Squat (Eccentric)', '25 reps, 5 min', '{"duration_min":5,"reps":25}'::jsonb, 'sissy-squat', '[]'::jsonb, '{}'::text[], 'knee_ability', 'dense', '["Lower to a high surface and stand back up.","Lower to a progressively lower surface.","Full range to the floor, controlled the whole way down."]'::jsonb, null, false, null, null),
  ('dense-nordic-curl-eccentric', 37, 'Nordic Curl (Eccentric)', '25 reps, 5 min — Lowering phase only — no concentric.', '{"duration_min":5,"reps":25}'::jsonb, 'nordic-hamstring-curl', '[{"equipment_missing":"nordic_support","note":"Planet Fitness: seated leg curl machine.","use_slug":"seated-leg-curl"},{"equipment_missing":"nordic_support","note":"Travelling: reverse Nordic.","use_slug":"reverse-nordic"}]'::jsonb, '{}'::text[], 'posterior_chain', 'dense', '["Lowering only, hands catch.","Slower lowering.","Full reps down and up."]'::jsonb, null, false, null, null),
  ('dense-ql-extension', 38, 'QL Extension', '3 × 10 reps, per side', '{"reps":10,"sets":3}'::jsonb, 'ql-extension', '[{"equipment_missing":"back_extension_bench","note":"Neither gym has a back-extension bench: the seated good morning covers the same low-back standard.","use_slug":"seated-good-morning"}]'::jsonb, '{}'::text[], 'posterior_chain', 'dense', '["Bodyweight, short range.","Full range.","Holding a plate."]'::jsonb, null, true, null, null),
  ('standards-plantar-fascia-stretch', 39, 'Plantar Fascia Stretch', '2 min', '{"duration_min":2}'::jsonb, 'foot-smr', '[]'::jsonb, '{}'::text[], 'warm_up', 'standards', '[]'::jsonb, null, false, null, null),
  ('standards-tibialis-stretch', 40, 'Tibialis Stretch', '2 min', '{"duration_min":2}'::jsonb, 'posterior-tibialis-stretch', '[]'::jsonb, '{}'::text[], 'warm_up', 'standards', '[]'::jsonb, null, false, null, null),
  ('standards-bodyweight-walk', 41, 'Bodyweight Walk', '0.25 mile', '{"distance_mi":0.25}'::jsonb, 'zone-2-steady', '[{"equipment_missing":"outdoor_route","note":"Planet Fitness or bad weather: treadmill at an easy pace.","use_slug":"walking-treadmill"}]'::jsonb, '{}'::text[], 'warm_up', 'standards', '[]'::jsonb, null, false, null, null),
  ('standards-single-leg-elevated-pike', 42, 'Single-Leg Elevated Pike', '3 sets, 60 s hold, per side', '{"hold_s":60,"sets":3}'::jsonb, 'leg-up-hamstring-stretch', '[]'::jsonb, '{}'::text[], 'hip_flexors_core', 'standards', '[]'::jsonb, null, true, null, null),
  ('standards-poliquin-step-up', 43, 'Poliquin Step-Up', '5 × 20 reps, per side — BENCHMARK: 66% BW, heel elevated, 3-4" box.', '{"pct_bodyweight":0.66,"reps":20,"sets":5}'::jsonb, 'poliquin-step', '[{"equipment_missing":"plyo_box","note":"No 3–4 inch box at either gym: a stair or a stacked pair of plates is the rise; the Patrick Step is the regression until one exists.","use_slug":"patrick-step"},{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"poliquin-step"}]'::jsonb, '{}'::text[], 'knee_ability', 'standards', '["Bodyweight off a low rise.","Heel elevated on the board.","Loaded toward the 66%-bodyweight standard."]'::jsonb, null, true, null, null),
  ('standards-jefferson-curl', 44, 'Jefferson Curl', '5 × 10 reps — Loaded spinal flexion — check it against the standing low-back injury before prescribing. — BENCHMARK: 25% BW for 10 reps.', '{"pct_bodyweight":0.25,"reps":10,"sets":5}'::jsonb, 'jefferson-curl', '[{"equipment_missing":"barbell","note":"No barbell at either gym: a single heavy dumbbell held in both hands works to about 50 lb.","use_slug":"jefferson-curl"},{"equipment_missing":"plyo_box","note":"No box: stand on the slant board or the end of a flat bench so the hands can pass below the toes.","use_slug":"jefferson-curl"}]'::jsonb, '{}'::text[], 'posterior_chain', 'standards', '["Unloaded roll-down off a box.","Light bar.","Toward 25% bodyweight for 10 reps."]'::jsonb, null, false, null, null),
  ('standards-hanging-leg-raise', 45, 'Hanging Leg Raise', '3 × 10 reps — BENCHMARK: Toes to bar.', '{"reps":10,"sets":3}'::jsonb, 'hanging-leg-raise', '[{"equipment_missing":"pull_up_bar","note":"Planet Fitness has no free-hanging bar: the captain’s-chair or a bench leg pull-in is the substitute.","use_slug":"leg-pull-in"}]'::jsonb, '{}'::text[], 'hip_flexors_core', 'standards', '["Bent-knee raise.","Straight-leg raise to horizontal.","Toes to bar."]'::jsonb, null, false, null, null),
  ('standards-tibialis-raise', 46, 'Tibialis Raise', '3 × 20 reps', '{"reps":20,"sets":3}'::jsonb, 'tibialis-raise', '[{"equipment_missing":"wall_space","note":"No wall to lean on: anchor a band low in front and loop it over the forefoot.","use_slug":"tibialis-raise-band"},{"equipment_missing":"tibialis_bar","note":"No tib bar at either gym: stand a dumbbell on end and pinch it between the feet.","use_slug":"tibialis-raise-dumbbell"}]'::jsonb, '{}'::text[], 'lower_legs', 'standards', '["Stand closer to the wall — less load, easier.","Step farther from the wall — more load, harder.","Add load: a dumbbell held between the feet, or a band over the forefoot.","Tib bar, once one is available."]'::jsonb, null, false, null, null),
  ('standards-incline-dumbbell-press', 47, 'Incline Dumbbell Press', '4 × 10 reps', '{"reps":10,"sets":4}'::jsonb, 'incline-dumbbell-press', '[]'::jsonb, '{}'::text[], 'upper_body', 'standards', '[]'::jsonb, null, false, null, null),
  ('standards-trx-face-pull', 48, 'TRX Face Pull', '4 × 10 reps', '{"reps":10,"sets":4}'::jsonb, 'face-pull', '[{"equipment_missing":"suspension_trainer","note":"No TRX at Planet Fitness: the cable tower does the same job at the same height.","use_slug":"face-pull"},{"equipment_missing":"cable_machine","note":"No TRX and no cable at home: a band pulled apart at eye height hits the same rear delts and lower traps.","use_slug":"band-pull-apart"}]'::jsonb, '{}'::text[], 'upper_body', 'standards', '[]'::jsonb, null, false, null, null),
  ('standards-pigeon', 49, 'Pigeon', '90 s hold, per side', '{"hold_s":90}'::jsonb, 'seated-piriformis-stretch', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'standards', '[]'::jsonb, null, true, null, null),
  ('standards-couch-stretch', 50, 'Couch Stretch', '90 s hold, per side', '{"hold_s":90}'::jsonb, 'couch-stretch', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'standards', '["Knee down, torso upright, hands on the floor.","Hands to the front thigh.","Hands to the hips.","Shoulders to the wall."]'::jsonb, null, true, null, null),
  ('standards-calf-stretch', 51, 'Calf Stretch', '60 s hold', '{"hold_s":60}'::jsonb, 'standing-calves-calf-stretch', '[]'::jsonb, '{}'::text[], 'warm_up', 'standards', '[]'::jsonb, null, false, null, null),
  ('standards-relaxed-lunge', 52, 'Relaxed Lunge', '3 sets, 60 s hold, per side', '{"hold_s":60,"sets":3}'::jsonb, 'kneeling-hip-flexor', '[]'::jsonb, '{}'::text[], 'knee_ability', 'standards', '[]'::jsonb, null, true, null, null),
  ('standards-atg-split-squat', 53, 'ATG Split Squat', '5 × 5 reps, per side — BENCHMARK: 25% BW per hand.', '{"pct_bodyweight":0.25,"per_hand":true,"reps":5,"sets":5}'::jsonb, 'atg-split-squat', '[{"equipment_missing":"adjustable_dumbbell","note":"Travelling: bodyweight, or a loaded backpack.","use_slug":"atg-split-squat"}]'::jsonb, '{}'::text[], 'knee_ability', 'standards', '["Front foot elevated, holding a rail for assistance.","Front foot elevated, no hands.","Flat ground, no hands, back knee to the floor.","Dumbbells in both hands, toward 25% bodyweight per hand."]'::jsonb, null, true, 30, '{"weekly_increment_pct":2.5}'::jsonb),
  ('standards-seated-good-morning', 54, 'Seated Good Morning', '5 × 10 reps — BENCHMARK: 50% BW, abs to bench.', '{"pct_bodyweight":0.5,"reps":10,"sets":5}'::jsonb, 'seated-good-morning', '[{"equipment_missing":"barbell","note":"Neither gym has a barbell: Planet Fitness has the Smith machine and a seated good-morning lever.","use_slug":"lever-seated-good-morning"},{"equipment_missing":"barbell","note":"Home: hold a single heavy dumbbell at the chest and hinge from the hips.","use_slug":"seated-good-mornings"}]'::jsonb, '{}'::text[], 'posterior_chain', 'standards', '["Bodyweight hinge, hands behind the head.","Light bar.","Toward 50% bodyweight, abs to the bench."]'::jsonb, null, false, null, null),
  ('standards-garhammer-raise', 55, 'Garhammer Raise', '3 × 10 reps — BENCHMARK: 10 reps, Level 2.', '{"reps":10,"sets":3}'::jsonb, 'hanging-oblique-knee-raise', '[{"equipment_missing":"pull_up_bar","note":"Planet Fitness: captain’s chair or bench leg pull-in.","use_slug":"leg-pull-in"}]'::jsonb, '{}'::text[], 'hip_flexors_core', 'standards', '["Level 1 — knees to 90°, short pull.","Level 2 — from 90°, curl the knees higher toward the chest for 10 reps."]'::jsonb, null, false, null, null),
  ('standards-single-leg-calf-raise', 56, 'Single-Leg Calf Raise', '3 × 10 reps, per side — BENCHMARK: 25% BW, 10 reps.', '{"pct_bodyweight":0.25,"reps":10,"sets":3}'::jsonb, 'single-leg-calf-raise', '[{"equipment_missing":"slant_board","note":"Seth owns a slant board, so the slant version is the prescription at home. Travelling or at Planet Fitness: the same movement off a stair edge or a stack of plates, accepting the shorter range.","use_slug":"single-leg-calf-raise"}]'::jsonb, '{}'::text[], 'lower_legs', 'standards', '["Bodyweight, floor.","Bodyweight, heel below a step or board.","Loaded — the benchmark is 25% bodyweight for 10 reps."]'::jsonb, null, true, null, null),
  ('standards-atg-dips', 57, 'ATG Dips', '4 × 12 reps', '{"reps":12,"sets":4}'::jsonb, 'chest-dip', '[{"equipment_missing":"dip_station","note":"Neither location has a dip station: bench dips, accepting the shorter range.","use_slug":"bench-dips"}]'::jsonb, '{}'::text[], 'upper_body', 'standards', '["Partial depth.","Full depth, shoulder below elbow.","Weighted."]'::jsonb, null, false, null, null),
  ('standards-shoulder-external-rotation', 58, 'Shoulder External Rotation', '4 × 12 reps, per side', '{"reps":12,"sets":4}'::jsonb, 'external-rotation', '[{"equipment_missing":"cable_machine","note":"Home: band external rotation, elbow pinned to the side.","use_slug":"external-rotation-with-band"}]'::jsonb, '{}'::text[], 'upper_body', 'standards', '[]'::jsonb, null, true, null, null),
  ('standards-butterfly-stretch', 59, 'Butterfly Stretch', '2 min', '{"duration_min":2}'::jsonb, 'butterfly-yoga-pose', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'standards', '[]'::jsonb, null, false, null, null),
  ('standards-seated-pancake', 60, 'Seated Pancake', '2 min', '{"duration_min":2}'::jsonb, 'the-straddle', '[]'::jsonb, '{}'::text[], 'mobility_cooldown', 'standards', '[]'::jsonb, null, false, null, null),
  ('standards-atg-squat', 61, 'ATG Squat', '5 × 20 reps — BENCHMARK: 25% BW, 20 reps.', '{"pct_bodyweight":0.25,"reps":20,"sets":5}'::jsonb, 'atg-squat', '[]'::jsonb, '{}'::text[], 'knee_ability', 'standards', '["Bodyweight to full depth.","Goblet-held load.","Toward 25% bodyweight for 20 reps."]'::jsonb, null, false, null, null),
  ('standards-nordic-curl', 62, 'Nordic Curl', '5 × 10 reps — BENCHMARK: 10 full reps (no hands).', '{"reps":10,"sets":5}'::jsonb, 'nordic-hamstring-curl', '[{"equipment_missing":"nordic_support","note":"Planet Fitness has no GHD and nothing safe to anchor the ankles under: run the seated leg curl machine instead.","use_slug":"seated-leg-curl"},{"equipment_missing":"nordic_support","note":"Travelling: reverse Nordic needs no anchor and keeps the eccentric quality.","use_slug":"reverse-nordic"}]'::jsonb, '{}'::text[], 'posterior_chain', 'standards', '["Lower a short way, hands catch early.","Lower farther each week, hands catch late.","Full lower, push back up with the hands.","Full rep down and up, no hands — the standard is 10."]'::jsonb, null, false, null, null),
  ('standards-low-cable-pull-in', 63, 'Low Cable Pull-In', '3 × 20 reps, per side — BENCHMARK: 50% BW, 20 reps.', '{"pct_bodyweight":0.5,"reps":20,"sets":3}'::jsonb, 'leg-pull-in', '[{"equipment_missing":"cable_machine","note":"Home has no cable stack: a band anchored low at the ankle is the substitute, and the 50%-bodyweight standard is not reachable there.","use_slug":"leg-pull-in"}]'::jsonb, '{}'::text[], 'hip_flexors_core', 'standards', '["Bodyweight leg pull-in.","Light cable.","Toward 50% bodyweight for 20 reps."]'::jsonb, null, true, null, null),
  ('standards-atg-shoulder-press', 64, 'ATG Shoulder Press', '4 × 10 reps', '{"reps":10,"sets":4}'::jsonb, 'dumbbell-shoulder-press', '[]'::jsonb, '{}'::text[], 'upper_body', 'standards', '[]'::jsonb, null, false, null, null),
  ('standards-chin-up', 65, 'Chin-Up', '4 × 10 reps', '{"reps":10,"sets":4}'::jsonb, 'chin-up', '[{"equipment_missing":"pull_up_bar","note":"Planet Fitness has no free bar: the assisted pull-up machine.","use_slug":"assisted-standing-chin-up"},{"equipment_missing":"pull_up_bar","note":"Planet Fitness alternative: lat pulldown.","use_slug":"cable-bar-lateral-pulldown"}]'::jsonb, '{}'::text[], 'upper_body', 'standards', '["Band- or machine-assisted.","Bodyweight.","Weighted."]'::jsonb, null, false, null, null),
  ('standards-atg-deadlift', 66, 'ATG Deadlift', '5 × 10 reps — BENCHMARK: 100% BW, 10 reps.', '{"pct_bodyweight":1,"reps":10,"sets":5}'::jsonb, 'barbell-deadlift', '[{"equipment_missing":"barbell","note":"Planet Fitness has no barbell and does not allow floor deadlifts: the Smith machine is the only route there.","use_slug":"smith-deadlift"},{"equipment_missing":"barbell","note":"Home tops out at 52.5 lb per hand — the ATG RDL with dumbbells is the stand-in, well short of the 100%-bodyweight standard.","use_slug":"atg-rdl"}]'::jsonb, '{}'::text[], 'posterior_chain', 'standards', '["Partial range from blocks.","Floor.","Standing on a platform for extra range.","Toward 100% bodyweight for 10 reps."]'::jsonb, null, false, null, null),
  ('standards-sissy-squat', 67, 'Sissy Squat', '5 × 20 reps', '{"reps":20,"sets":5}'::jsonb, 'sissy-squat', '[]'::jsonb, '{}'::text[], 'knee_ability', 'standards', '["One-arm assisted.","Free-standing.","Loaded."]'::jsonb, null, false, null, null),
  ('standards-l-sit', 68, 'L-Sit', '3 sets, 15 s hold', '{"hold_s":15,"sets":3}'::jsonb, 'l-sit', '[]'::jsonb, '{}'::text[], 'hip_flexors_core', 'standards', '["Level 1 — alternate lifting one leg at a time, seated, for the full time.","Level 2 — same, with the hips off the floor.","Level 3 — full L-sit, both legs and hips off the floor."]'::jsonb, null, false, null, null),
  ('standards-bench-pullover', 69, 'Bench Pullover', '4 × 10 reps — BENCHMARK: 25% BW.', '{"pct_bodyweight":0.25,"reps":10,"sets":4}'::jsonb, 'bent-arm-dumbbell-pullover', '[]'::jsonb, '{}'::text[], 'upper_body', 'standards', '["Along the bench.","Across the bench, hips low, for the full overhead stretch.","Toward 25% bodyweight."]'::jsonb, null, false, null, null),
  ('standards-trap-raise', 70, 'Trap Raise', '4 × 10 reps', '{"reps":10,"sets":4}'::jsonb, 'dumbbell-shrug', '[]'::jsonb, '{}'::text[], 'upper_body', 'standards', '[]'::jsonb, null, false, null, null);
...
begin
  ...
  if v_steps <> 70 then
    raise exception 'kot: expected 70 steps, found %', v_steps;
  end if;
  if v_phases <> 3 then
